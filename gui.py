...
class HerpGuardGUI:
    ...
    def _build_form(self) -> None:
        ...
        analyze_button.grid(row=8, column=0, sticky="w", padx=12, pady=10)

        output_label = tk.Label(self.root, text="Monitoring Report")
        output_label.grid(row=9, column=0, sticky="nw", padx=12, pady=(4, 2))
        self.output_text = tk.Text(self.root, width=70, height=18, state="disabled")
        self.output_text.grid(row=9, column=1, sticky="we", padx=12, pady=(4, 12))

        self.root.grid_columnconfigure(1, weight=1)

    def _add_species_field(self, row: int) -> None:
        label = tk.Label(self.root, text="Species")
        label.grid(row=row, column=0, sticky="w", padx=12, pady=4)

        species_options = list_available_species()
        if species_options:
            combo = ttk.Combobox(self.root, values=species_options, state="readonly", width=37)
            combo.current(0)
        else:
            combo = ttk.Combobox(self.root, values=[], state="normal", width=37)

        combo.grid(row=row, column=1, sticky="we", padx=12, pady=4)
        self.species_combo = combo
    ...
    def _collect_input(self) -> dict[str, str]:
        return {
            "species": self.species_combo.get().strip(),
    ...
    def _analyze(self) -> None:
        ...
        if not inputs.get("species"):
            messagebox.showwarning("Missing Data", "Please select a supported species.")
            return
